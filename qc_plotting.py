...
def count_ars(raw, landfall, events, points):
    ...
    print(
        "AR detection was performed between "
        + str(start_year)
        + " and "
        + str(end_year)
        + ", in an area from latitude "
        + str(bbox[0])
        + " to "
        + str(bbox[2])
        + " and longitude "
    ...
    print(
        "There were "
        + str(len(r))
        + " individual timestep ARs detected across the entire spatial and temporal domain."
    )
    print(
        "Of these, "
        + str(len(l))
        + " of the detected ARs intersect the Alaska polygon boundary."
    )
    print(
        "Of these, "
        + str(len(e))
        + " possible AR events were aggregated from landfalling ARs, using a combination of adjacent timesteps and overlapping geometry to define an event."
    )
    print(
        str(len(p))
        + " AR events intersected the coastline along their primary axis of travel."
    )
    print(
        str(len(e) - len(p))
        + " AR events made landfall without their primary axis of travel did not intersect the coastline."
    )
    print("\n")
    print(
...
def agg_by_hexgrid_cells_and_year_month(hexgrid, hexid, gdf, prop, agg, allow_na):
    ...
    if allow_na == False:
        pt = gdf.pivot_table(
            index="Month", columns="Year", values=prop, aggfunc=agg
        ).fillna(0)
    else:
...
def plot_3panel_heatmaps(
    ...
    grid = plt.GridSpec(
        2, 2, wspace=0.05, hspace=-0.35, height_ratios=[1, 2.5], width_ratios=[1, 1.57]
    )
    ...
    if dev == True:
        fulldivnorm = mpl_colors.TwoSlopeNorm(
            vmin=hexdf_full[prop].min(), vcenter=0.01, vmax=hexdf_full[prop].max()
        )
        coastdivnorm = mpl_colors.TwoSlopeNorm(
            vmin=hexdf_coast[prop].min(), vcenter=0.01, vmax=hexdf_coast[prop].max()
        )
    ...
    heatmap_ax.set_xticklabels(
        labels=[str(y) for y in pt.columns.to_list()], rotation=45, fontsize=f
    )
    ...
    colorbar = fig.colorbar(
        heatmap_ax.collections[0],
        location="top",
        ticks=[pt_vmin, pt_vmax],
        shrink=0.35,
        pad=0.18,
    )
    colorbar.set_ticklabels(
        [cbar_labels[0], cbar_labels[1]], fontsize=f - 1, fontstyle="italic"
    )
    ...
    fig.suptitle(title, y=0.895, fontsize=f + 3)

    # enso legend
    if type(phenom) != str:
        plt.figtext(0.1, 0.9, phenom_string, fontsize=f - 1)
    else:
        pass
...
def events_prop_mmm(events):
    ...
    fig, axes = plt.subplots(
        int(len(sub.drop(columns="start").columns) / 2), 2, sharex=False, figsize=(8, 7)
    )
    ...
    for attr, ax, t, c in zip(
        sub.drop(columns="start").columns.values.tolist(), fig.axes, titles, colors
    ):
        # get stats
        a_min, a_mean, a_max = (
            sub[attr].values.min(),
            sub[attr].values.mean(),
            sub[attr].values.max(),
        )
        # get time values for each stat
        a_min_idx, a_max_idx = sub[attr].idxmin(), sub[attr].idxmax()
        min_t, max_t = (
            sub["start"].iloc[sub[attr].idxmin()],
            sub["start"].iloc[sub[attr].idxmax()],
        )
        ...
        sns.lineplot(
            ax=ax,
            x=[mean_t_low, mean_t, mean_t_high],
            y=[a_mean, a_mean, a_mean],
            color=c,
            linewidth=0.6,
            linestyle=(0, (3, 3)),
        )
        # plot min/mean/max points
        ax.plot(min_t, a_min, marker="o", fillstyle="none", markeredgewidth=0.5, c=c)
        ax.plot(max_t, a_max, marker="o", fillstyle="full", markeredgewidth=0.5, c=c)
        ax.plot(
            mean_t, a_mean, marker="o", fillstyle="bottom", markeredgewidth=0.5, c=c
        )
        ...
        ax.text(
            x=min_t, y=a_min + nudge, s=str("  " + str(int(a_min))), color=c, fontsize=f
        )
        ax.text(
            x=max_t,
            y=a_max - nudge * 3,
    ...
    fig.suptitle(
        (
            "AR event properties: min, mean, max ("
            + str(start_year)
            + "-"
            + str(end_year)
            + ")\nn = "
...
def events_pairplot(events):
    ...
    """
    sub = events[
        ["dur_hrs", "rintensity", "tintensity", "ratio_m", "len_km_m", "dircoher_m"]
    ].copy()
    sub.rename(
        columns={
            "dur_hrs": "duration (hrs)",
            "rintensity": "relative intensity",
            "tintensity": "total intensity",
            "ratio_m": "length/width ratio",
            "len_km_m": "length (km)",
            "dircoher_m": "mean directional coherence (%)",
        },
        inplace=True,
    )
    sns.set_style("white")
